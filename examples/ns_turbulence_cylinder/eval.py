...
def evaluate(config: ml_collections.ConfigDict, workdir: str):
    ...
    if config.nondim == True:
        ...
        U_star = 9.0  # characteristic velocity #zomo
        L_star = 80.0  # characteristic length
        ...
        symmetry_coords = symmetry_coords / L_star
    ...
    plt.subplot(3, 2, 1)
    plt.tricontourf(triang, u_pred[0], cmap='jet', levels=100)
    plt.colorbar()
    plt.title('Predicted $u$')
    plt.tight_layout()

    plt.subplot(3, 2, 2)
    plt.tricontourf(triang, v_pred[0], cmap='jet', levels=100)
    plt.colorbar()
    plt.title('Predicted $v$')
    plt.tight_layout()

    plt.subplot(3, 2, 3)
    plt.tricontourf(triang, p_pred[0], cmap='jet', levels=100)
    plt.colorbar()
    plt.title('Predicted $p$')
    plt.tight_layout()

    plt.subplot(3, 2, 4)
    plt.tricontourf(triang, k_pred[0], cmap='jet', levels=100)
    plt.colorbar()
    plt.title('Predicted $k$')
    plt.tight_layout()

    plt.subplot(3, 2, 5)
    plt.tricontourf(triang, omega_pred[0], cmap='jet', levels=100)
    plt.colorbar()
