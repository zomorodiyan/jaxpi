...
class NavierStokes2D(ForwardIVP):
    ...
    def r_net(self, params, t, x, y):
        ...
        debug.print(" u,v,k,omega_t: {}, {}, {}, {}", u_t,v_t,k_t,omega_t)
        ...
        debug.print(" u,v,p,k,omega_x: {}, {}, {}, {}, {}", u_x,v_x,p_x,k_x,omega_x)
        ...
        debug.print(" u,v,p,k,omega_y: {}, {}, {}, {}, {}", u_y,v_y,p_y,k_y,omega_y)
        ...
        debug.print(" u_xx,u_xy,v_yx,v_yy: {}, {}, {}, {}", u_xx,u_xy,v_yx,v_yy)
        ...
        k = self.positive(k)
        phi_11 = jnp.sqrt(k) / (0.09 * omega * y_hat)
        phi_12 = 500 / (Re * y_hat**2 * omega)
        phi_13 = 4 * k / (sigma_omega2 * D_omega_plus * y_hat**2)
        phi_1 = jnp.minimum(jnp.maximum(phi_11, phi_12), phi_13)
        phi_21 = (2 * jnp.sqrt(k)) / (0.09 * omega * y_hat)
        phi_22 = 500 / (Re * y_hat**2 * omega)
        phi_2 = jnp.maximum(phi_21, phi_22)

        #debug.print("phi_11_12_13_1: {}, {}, {}, {}", phi_11, phi_12, phi_13, phi_1)
        #debug.print("phi_21_22_2: {}, {}, {}", phi_21, phi_22, phi_2)
        debug.print(" y_hat,D_omega_plus,phi_1,phi_2: {}, {}, {}, {}", y_hat,D_omega_plus,phi_1,phi_2)
        ...
        debug.print(" F1,F2,beta_i,alpha_infinity_1,alpha_infinity_2: \
                   {}, {}, {}, {}, {}", F1,F2,beta_i,alpha_infinity_1,alpha_infinity_2)
        ...
        debug.print(" Re_t,alpha_star,alpha,beta_star_i,M_t, F_Mt: \
                   {}, {}, {}, {}, {}, {}", Re_t,alpha_star,alpha,beta_star_i,M_t, F_Mt)
    ...
    def res_and_w(self, params, batch):
        ...
        debug.print("t_sorted_5_vals and shape): {}, {}", t_sorted[:5],\
                    t_sorted.shape)
        ...
        #debug.print("continuity_pred: {}", continuity_pred[:5])
        #debug.print("x_momentum_pred: {}", x_momentum_pred[:5])
        #debug.print("y_momentum_pred: {}", y_momentum_pred[:5])
        #debug.print("k_transport_pred: {}", k_transport_pred[:5])
        #debug.print("omega_transport_pred: {}", omega_transport_pred[:5])
        ...
        #debug.print("continuity_l: {}", continuity_l)
        #debug.print("x_momentum_l: {}", x_momentum_l)
        #debug.print("y_momentum_l: {}", y_momentum_l)
        #debug.print("k_transport_l: {}", k_transport_l)
        #debug.print("omega_transport_l: {}", omega_transport_pred)
    ...
    def compute_drag_lift(self, params, t, U_star, L_star):
        nu = 1.7894e-5/1.225  # kinematic viscosity
...
class NavierStokesEvaluator(BaseEvaluator):
    ...
    def log_preds(self, params, x_star, y_star):
        ...
        k_pred = vmap(vmap(model.k_net, (None, None, 0)), (None, 0, None))(params, x_star, y_star)
        omega_pred = vmap(vmap(model.omega_net, (None, None, 0)), (None, 0, None))(params, x_star, y_star)
